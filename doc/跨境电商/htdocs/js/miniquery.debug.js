//文件 miniquery.debug.js 由 ant 合并生成于 2014-05-29 18:12:09


//================================================================================================================>
//开始 miniquery.debug.js
;( function (
    global, 
    Array, 
    Boolean,
...
* @namespace
*/
MiniQuery.Class = (function ($, This) {


return $.extend(This, { /**@lends MiniQuery.Class*/
    ...
    */
    create: function (instances, statics) {
        ...
        $.extend(prototype.constructor, statics); //静态成员
    ...
    */
    inherit: function (SuperClass, instances, statics) {
        instances = $.extend(new SuperClass(), instances);
        statics = $.extend({}, SuperClass, statics);

        return This.create(instances, statics);
    }
...
$.extend(This, { /**@lends MiniQuery.Event*/
    ...
    bind: function (obj, eventName, handler, isOnce) {
        ...
        function bindItem(eventName, handler) {
            if (typeof handler != 'function') {
                throw new Error('参数 handler 必须为一个 function');
            }
    ...
    * @param {function} [handler] 要检测的事件处理函数。
    * @return 返回一个布尔值，该布尔值指示目标对象上是否包含指否类型的事件以及指定的处理函数。
        如果是则返回 true；否则返回 false。
    */
    has: function (obj, eventName, handler) {
    ...
    * @example
        ...
        $.Event.bind(obj, 'show', handler);
        $.Event.trigger(obj, 'show'); //输出 100

        $.Event.toggle(obj, 'show', handler); //因为 'show' 事件列表中已存在 handler 函数，所以会给移除
        $.Event.trigger(obj, 'show'); //handler 函数已给移除，因为不产生输出
    */
    toggle: function (obj, eventName, handler, isOnce) {

        if (This.has(obj, eventName, handler)) {
            This.unbind(obj, eventName, handler);
        }
        else {
            this.bind(obj, eventName, handler, isOnce);
        }
    }
...
This.prototype = { /**@inner*/
    ...
    has: function (eventName, handler) {
        return This.has(this.value, eventName, handler);
    },

    toggle: function (eventName, handler, isOnce) {
        This.toggle(this.value, eventName, handler, isOnce);
        return this;
    }
...
$.extend(States.prototype, { /**@lends MiniQuery.States#*/
    ...
    */
    to: function (name, args) {
        ...
        mapper.get(this).histories.push(name);
...
return $.extend(This, {  /**@lends MiniQuery.Url */
    ...
    * @example
        $.Url.Current.onhashchange(top, function (newHash, oldHash) {
    ...
    */
    onhashchange: function (window, fn) {
    ...
    */
    route: function (window, routes) {
        ...
        This.onhashchange(window, function (newHash, oldHash) {
...
return {  /**@lends MiniQuery.Url.Current */
    ...
    */
    getQueryString: function (key, ignoreCase) {
        ...
        var args = [location.href].concat($.Array.parse(arguments));
    ...
    * @example
        $.Url.Current.onhashchange(function (newHash, oldHash) {
            console.log('new hash: ' + newHash);
            console.log('old hash: ' + oldHash);
    ...
    */
    onhashchange: function (fn) {
        ...
        var args = [window].concat($.Array.parse(arguments));
        Url.onhashchange.apply(null, args);
    },
